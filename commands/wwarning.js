...
module.exports = {
    ...
    execute(message, args){
        ...
        if (message.member.roles.cache.some(role =>['NIS', 'Company Command', 'Regiment Command', 'Platoon Command', 'NIS Command'].includes(role.name))) {
...
if(!reason42) return message.channel.send(`${message.author}, Please specify a reason for the written warning.`);

        logchannel1.send(`${args[0]} has been given a Written Warning for **${args.slice(1).join(' ')}**. \n\n**Issued by:** ${message.author}\n<@&821910794307502101>`)

message.delete()
            
        const modlogs = new Discord.MessageEmbed()
        .setColor('GREEN')
        .setTitle('Written Warning Command')
        .setAuthor('CFMP Moderation Logs', 'https://cdn.discordapp.com/emojis/799646515496747069.png?v=1')
        .addFields(
            { name: 'Command Usage', value: `**${message.content}**` },
            { name: 'Command Used', value: '**-wwarning**', inline: true },
    ...
    let modchannel1 = message.client.channels.cache.get(process.env.modchannel);
        modchannel1.send(modlogs);
            ...
            message.channel.send(`${message.author}, Insufficient Permissions for **-wwarning** command.\n\nRoles required: **NIS**, **Platoon Command**, **Company Command**, or **Regiment Command**`);
        }
